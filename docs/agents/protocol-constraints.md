...
- 这条协议只约束共享 prelude 面；英雄私有变量不必继续挤在 prelude 中。变量归属、迁移和基线边界以 `docs/agents/variable-ownership.md` 为准。
- `tools/data/contract-guard/protocol-indexes.tsv` 只记录仍属于共享 prelude ABI 的声明，不是所有 `globalvar` / `playervar` 的总注册表。
...
- 移出 prelude 的变量必须保留逻辑名称；如移除会占用共享协议槽位，应在所有需要的 owner 文件中保留原显式索引，避免无关共享变量重编号。
