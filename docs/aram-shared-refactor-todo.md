...
- H9 Wave-A/B/C（2026-03-08）：修正 contract 顺序源（`modules/_index.opy` 与 `debug/_index.opy` 收敛旧 include），使 `src/main.opy` 的 `#!optimizeStrict` 后区段与当前 baseline 一致并清除 `strict-hero-init` include mismatch；复核 duplicates 基线不变，TODO 进入稳定维护模式。
...
- 波次范围：
  - contract 修复：调整 `src/modules/_index.opy` / `src/modules/debug/_index.opy` 的展开顺序源，让 `src/main.opy` 的 `#!optimizeStrict` 后区段与当前 baseline 对齐，并消除 `strict-hero-init` include mismatch 噪声
